 ...
 * @note    The driver GPTD4 allocates the timer CT32B0 when enabled.
 */
 ...
 * @note    The driver GPTD5 allocates the complex timer CT32B1 when enabled.
 */
 ...
 * @note    The driver GPTD6 allocates the timer CT32B2 when enabled.
 */
...
void gpt_lld_init(void) {
  ...
  GPTD1.ct16 = SN32_CT16B0;
  gptObjectInit(&GPTD1);
#endif

#if SN32_GPT_USE_CT16B1
  /* Driver initialization.*/
  GPTD2.ct16 = SN32_CT16B1;
  gptObjectInit(&GPTD2);
#endif

#if SN32_GPT_USE_CT16B2
  /* Driver initialization.*/
  GPTD3.ct16 = SN32_CT16B2;
  gptObjectInit(&GPTD3);
#endif

#if SN32_GPT_USE_CT32B0
  /* Driver initialization.*/
  GPTD4.ct32 = SN32_CT32B0;
  gptObjectInit(&GPTD4);
#endif

#if SN32_GPT_USE_CT32B1
  /* Driver initialization.*/
  GPTD5.ct32 = SN32_CT32B1;
  gptObjectInit(&GPTD5);
#endif

#if SN32_GPT_USE_CT32B2
  /* Driver initialization.*/
  GPTD6.ct32 = SN32_CT32B2;
  gptObjectInit(&GPTD6);
#endif
}
...
void gpt_lld_start(GPTDriver *gptp) {
  ...
  if (gptp->state == GPT_STOP) {
    ...
    if (&GPTD4 == gptp) {
      ...
      CT32B0_ResetTimer();
#if !defined(SN32_CT32B0_SUPPRESS_ISR)
      nvicEnableVector(SN32_CT32B0_NUMBER, SN32_GPT_CT32B0_IRQ_PRIORITY);
#endif
    gptp->clock = SystemCoreClock;
    }
#endif
    ...
    if (&GPTD5 == gptp) {
      ...
      CT32B1_ResetTimer();
#if !defined(SN32_CT32B1_SUPPRESS_ISR)
      nvicEnableVector(SN32_CT32B1_NUMBER, SN32_GPT_CT32B1_IRQ_PRIORITY);
#endif
    gptp->clock = SystemCoreClock;
    }
#endif
    ...
    if (&GPTD6 == gptp) {
      ...
      CT32B2_ResetTimer();
#if !defined(SN32_CT32B2_SUPPRESS_ISR)
      nvicEnableVector(SN32_CT32B2_NUMBER, SN32_GPT_CT32B2_IRQ_PRIORITY);
#endif
    gptp->clock = SystemCoreClock;
    }
#endif
  ...
  /* Timer configuration for CT16.*/
  gptp->ct16->TMRCTRL = CT16_CEN_DIS;            /* Initially stopped.       */
  gptp->ct16->CNTCTRL = gptp->config->cntctrl;
  gptp->ct16->PRE  = psc;                        /* Prescaler value.         */
  gptp->ct16->IC   &= 0x1FFFFFF;                 /* Clear pending IRQs.      */

  /* Timer configuration for CT32.*/
  gptp->ct32->TMRCTRL = CT32_CEN_DIS;            /* Initially stopped.       */
  gptp->ct32->CNTCTRL = gptp->config->cntctrl;
  gptp->ct32->PRE  = psc;                        /* Prescaler value.         */
  gptp->ct32->IC   &= 0x1FFFFFF;                 /* Clear pending IRQs.      */
}
...
void gpt_lld_stop(GPTDriver *gptp) {
  ...
  if (gptp->state == GPT_READY) {
    /* CT16 */
    gptp->ct16->TMRCTRL = CT16_CEN_DIS;          /* Timer disabled.          */
    gptp->ct16->IC   &= 0x1FFFFFF;               /* Clear pending IRQs.      */

    /* CT32 */
    gptp->ct32->TMRCTRL = CT32_CEN_DIS;          /* Timer disabled.          */
    gptp->ct32->IC   &= 0x1FFFFFF;               /* Clear pending IRQs.      */
...
void gpt_lld_start_timer(GPTDriver *gptp, gptcnt_t interval) {
  ...
  /* CT16 */
  gptp->ct16->MR0 = (uint32_t)(interval - 1U);  /* Time constant.           */
  gptp->ct16->TMRCTRL = mskCT16_CRST;           /* Reset counter.           */
  gptp->ct16->IC      &= 0x1FFFFFF;             /* Clear pending IRQs.      */
  if (NULL != gptp->config->callback)
    gptp->ct16->MCTRL |= mskCT16_MR0IE_EN;
  gptp->ct16->TMRCTRL |= mskCT16_CEN_EN;

  /* CT32 */
  gptp->ct32->MR0 = (uint32_t)(interval - 1U);  /* Time constant.           */
  gptp->ct32->TMRCTRL = mskCT32_CRST;           /* Reset counter.           */
  gptp->ct32->IC      &= 0x1FFFFFF;             /* Clear pending IRQs.      */
  if (NULL != gptp->config->callback)
    gptp->ct32->MCTRL |= mskCT32_MR0IE_EN;
  gptp->ct32->TMRCTRL |= mskCT32_CEN_EN;
}
...
void gpt_lld_stop_timer(GPTDriver *gptp) {
  ...
  gptp->ct16->TMRCTRL = CT16_CEN_DIS;            /* Initially stopped.       */
  gptp->ct16->IC      &= 0x1FFFFFF;              /* Clear pending IRQs.      */
  gptp->ct16->MCTRL &= ~mskCT16_MR0IE_EN;        /* Disable the interrupt    */

  /* CT32 */
  gptp->ct32->TMRCTRL = CT32_CEN_DIS;            /* Initially stopped.       */
  gptp->ct32->IC      &= 0x1FFFFFF;              /* Clear pending IRQs.      */
  gptp->ct32->MCTRL &= ~mskCT32_MR0IE_EN;        /* Disable the interrupt    */
}
...
void gpt_lld_polled_delay(GPTDriver *gptp, gptcnt_t interval) {
  ...
  gptp->ct16->MR0 = (uint32_t)(interval - 1U);   /* Time constant.           */
  gptp->ct16->IC &= 0x1FFFFFF;                   /* Clear pending IRQs.      */
  gptp->ct16->MCTRL = (mskCT16_MR0IE_EN | mskCT16_MR0STOP_EN);
  gptp->ct16->TMRCTRL |= mskCT16_CEN_EN;
  while (!(gptp->ct16->RIS & mskCT16_MR0IF))
    ;
  gptp->ct16->IC &= 0x1FFFFFF;                   /* Clear pending IRQs.      */
  ...
  gptp->ct32->MR0 = (uint32_t)(interval - 1U);   /* Time constant.           */
  gptp->ct32->IC &= 0x1FFFFFF;                   /* Clear pending IRQs.      */
  gptp->ct32->MCTRL = (mskCT32_MR0IE_EN | mskCT32_MR0STOP_EN);
  gptp->ct32->TMRCTRL |= mskCT32_CEN_EN;
  while (!(gptp->ct32->RIS & mskCT16_MR0IF))
    ;
  gptp->ct32->IC &= 0x1FFFFFF;                   /* Clear pending IRQs.      */
}
...
void gpt_lld_serve_interrupt(GPTDriver *gptp) {
  ...
  gptp->ct16->IC &= 0x1FFFFFF;
  gptp->ct32->IC &= 0x1FFFFFF;
