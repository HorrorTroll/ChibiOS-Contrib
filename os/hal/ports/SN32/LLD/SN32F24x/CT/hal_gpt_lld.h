...
#if !SN32_GPT_USE_CT32B0 && !SN32_GPT_USE_CT32B0 &&                           \
    !SN32_GPT_USE_CT32B1 && !SN32_GPT_USE_CT32B1 &&                           \
    !SN32_GPT_USE_CT32B2 && !SN32_GPT_USE_CT32B2
#error "GPT driver activated but no CT32 peripheral assigned"
#endif
...
struct GPTDriver {
  ...
  uint32_t                  clock;
  /**
   * @brief Pointer to the CT16 registers block.
   */
  sn32_ct16_t               *ct16;
  /**
   * @brief Pointer to the CT32 registers block.
   */
  sn32_ct32_t               *ct32;
};
...
#define gpt_lld_change_interval(gptp, interval)                             \
  ((gptp)->ct16->MR0 = (uint32_t)((interval) - 1U))                         \
  ((gptp)->ct32->MR0 = (uint32_t)((interval) - 1U))
...
#define gpt_lld_get_interval(gptp)                             \
  ((gptcnt_t)((gptp)->ct16->MR0 + 1U))                         \
  ((gptcnt_t)((gptp)->ct32->MR0 + 1U))
...
#define gpt_lld_get_counter(gptp)                             \
  ((gptcnt_t)(gptp)->ct16->TC)                                \
  ((gptcnt_t)(gptp)->ct32->TC)
