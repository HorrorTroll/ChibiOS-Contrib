...
/**
 * @file    hal_efl_lld.c
 * @brief   AT32F40xx Embedded Flash subsystem low level driver source.
 *
 * @addtogroup HAL_EFL
 * @{
 */
...
static inline void at32_flash_lock(EFlashDriver *eflp) {
  ...
  eflp->flash->CTRL2 |= FLASH_CTRL2_OPLK;
}
...
static inline void at32_flash_unlock(EFlashDriver *eflp) {
  ...
  eflp->flash->UNLOCK2 |= FLASH_KEY1;
  eflp->flash->UNLOCK2 |= FLASH_KEY2;
}

static inline void at32_flash_enable_pgm(EFlashDriver *eflp) {

  eflp->flash->CTRL |= FLASH_CTRL_FPRGM;
  eflp->flash->CTRL2 |= FLASH_CTRL2_FPRGM;
}
...
static inline flash_error_t at32_flash_check_errors(EFlashDriver *eflp) {
  ...
  uint32_t sts2 = eflp->flash->STS2;

  /* Clearing error conditions.*/
  eflp->flash->STS = sts & 0x0000001FU;
  eflp->flash->STS2 = sts2 & 0x0000001FU;
...
flash_error_t efl_lld_start_erase_sector(void *instance,
  ...
  devp->flash->CTRL2 |= FLASH_CTRL2_SECERS;
  ...
  devp->flash->CTRL2 |= FLASH_CTRL2_ERSTR;
...
flash_error_t efl_lld_query_erase(void *instance, uint32_t *wait_time) {
  ...
  if (devp->state == FLASH_ERASE) {
    ...
    if (at32_flash_is_busy(devp) == 0U) {
      ...
      devp->flash->CTRL2 &= ~(FLASH_CTRL2_BANKERS | FLASH_CTRL2_SECERS);
